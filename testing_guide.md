...
- Observing the new headers pre-synchronization phase during IBD ([#25717](https://github.com/bitcoin/bitcoin/pull/25717))
...
- Testing transient addresses for I2P outbound connections ([#25355](https://github.com/bitcoin/bitcoin/pull/25355))
...
- Testing watch-only support for Miniscript descriptors. ([#24148](https://github.com/bitcoin/bitcoin/pull/24148))

For a comprehensive list of changes in Bitcoin Core 24.0, check out the [release notes][release notes].

## Preparation

### 1. Grab Latest Release Candidate

**Current Release Candidate:** [Bitcoin Core 24.0rc1][source-code] ([release-notes][release notes])
...
[source-code]: https://github.com/bitcoin/bitcoin/releases/tag/v24.0rc1
[binaries]: https://bitcoincore.org/bin/bitcoin-core-24.0/test.rc1/

### 2. Compile Release Candidate

**If you grabbed a binary, skip this step**

Before compiling, make sure that your system has all the right [dependencies][dependencies] installed. As this guide utilizes the Bitcoin Core GUI, you must **compile with support for the GUI**(`--with-gui=yes`) and have the `qt5` dependency already installed. Since descriptor wallets are now the default, it is recommended that you **compile with sqlite**(`--with-sqlite=yes`), so make sure you have installed the `sqlite3` dependency. If you are planning to [test the new migration mechanism](#migrating-legacy-wallets-to-descriptor-wallets), you must **compile with support for legacy wallets**, so make sure you have the Berkeley DB(`bdb`) dependency installed.
...
Several sections of this guide will ask you to send/receive funds to your wallet.
When needed, guidance for creating a wallet and generating addresses will be given.

If you are testing on signet, you can use test coins for the tests in this guide. After IBD has completed (this process is relatively quick on signet), you can get some free corn from one of the signet faucets available, e.g. https://signetfaucet.com/. Generate a fresh address, pop it into the faucet, and [watch the signet coins flowing in](https://mempool.space/signet).
...
> :bulb: You can see all the available options for a command using `cli [cli options] help <command>`.
>
> For example: `cli help createwallet`.

When you have finished testing, please [return your coins to a faucet in need](https://signet.bublina.eu.org/about.html) before stopping your node and cleaning up your datadir.

> :bulb: An efficent way to sweep your testing wallet is the new `sendall` RPC ([#24118](https://github.com/bitcoin/bitcoin/pull/24118)).
...
> :book: **_What is `nMinimumChainWork`?_** A number designed to protect new clients from accepting fake blockchains during initial sync. It's [updated on every release](https://github.com/bitcoin/bitcoin/pull/25946) and defines the minimum amount of total work a chain must have before the client considers it valid.
...
> :information_source: You can also see real-time logging in the console by using the `-printtoconsole` flag.
...
> :information_source: You can test a localization using `-lang=<lang>`.
...
<img width="450" alt="gui shortcut ambiguity" src="https://user-images.githubusercontent.com/18506343/191549555-afcfd791-e21c-47ea-b63e-38b34cee0cdb.gif">

This is a pretty simple test - **navigate in the GUI, use the shortcuts and see if you can find any more ambiguities**.

This kind of issues are responsibility of the translators. If you want to directly help with a fix, see [doc/translation_process.md](https://github.com/bitcoin-core/gui/blob/master/doc/translation_process.md). The best way to report this is to open an issue on [Bitcoin Core's Transifex page](https://www.transifex.com/bitcoin/bitcoin/), see how on [Comments and Issues section on Transifex docs](https://docs.transifex.com/translation/tools-in-the-editor#comments-and-issues). Alternatively, comment [here](https://github.com/bitcoin-core/gui/issues/126) with details.
...
_Note: For this test you will also need bitcoind._
...
> :bulb: Instead of scanning through the lengthy response of `getpeersinfo` in order to confirm your address for each outgoing connection, you can **use some `jq` magic to get a simplified view**:
>
> ```shell
...
> :information_source: The HD seed is a valid key that could receive Bitcoin to it even though its corresponding addresses would never be given out.
...
> :bulb: Multisigs are special, `addmultisigaddress` adds a `redeemscript` so it's ****solvable****, but it's ****not** marked watch-only**. The net-effect of that is that we see the full script with `getaddressinfo`, **the wallet ignores transactions to it**, but we can sign them. This may be useful if you're a co-signer and you don't want those transactions to show up.
...
Thank you for your help in making Bitcoin as robust as it can be. Please remember to add a comment on [v24.0 testing issue detailing](https://github.com/bitcoin/bitcoin/issues/26175):
...
[miniscript-example]: https://miniscript.fun/#
